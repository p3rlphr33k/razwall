			...
			DragResize.prototype.mouseMove=function(e)
				...
				with(this)
					...
					with(element.style)
						...
						$('indicator').innerHTML='Moving: '+top+','+left+' Resizing: '+width+','+height;
						}
...
DragResize.prototype.resizeHandleDrag=function(diffX,diffY)
			{
				with(this)
					{
					var hClass=handle&&handle.className&&handle.className.match(new RegExp(myName+'-([tmblr]{2})'))?RegExp.$1:'';var dY=diffY,dX=diffX,processed=false;
					...
					if(hClass.indexOf('r')>=0)
						...
						elmW+=diffX;processed=true}return processed
						}
				};

var dragresize = new DragResize('dragresize',{ minWidth: 400, minHeight: 200, minLeft: 50, minTop: 55}); //, maxLeft: window.innerWidth-5, maxTop: window.innerHeight-5 });

...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var eg_classes = 'ui-jqgrid ui-widget ui-widget-content ui-corner-all';
		if (ts.p.isSubGrid===true) {
		    eg_classes = '';
		}
		var eg = $("<div class='"+eg_classes+"'></div>");
		...
		addMulti = function(rowid,pos,irow,checked){
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"ID\" value=\""+rowid+"\" />",
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-widget-header ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		$(grid.hDiv)
			...
			.addClass("ui-widget-header ui-jqgrid-hdiv")
			.append(hb);
		$(hb).append(hTable);
		hTable = null;
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: grid.width+"px"}).appendTo(eg).addClass('ui-widget-header ui-jqgrid-pager ui-corner-bottom');
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(){
				...
				th = $("<th role='columnheader' class='ui-widget-header ui-th-column ui-th-"+$t.p.direction+"'></th>");
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						.addClass("ui-widget-header ui-th-column-header ui-th-"+ts.p.direction)
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
							atd = pos >=1 ? "<td class='ui-widget-content subgrid-cell' colspan='"+pos+"'>&#160;</td>":"";

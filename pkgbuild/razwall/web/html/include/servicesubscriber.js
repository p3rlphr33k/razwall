...
var SERVICE_BASE = "/manage/status/status.notifications.services/";
//var SERVICE_STATUSLOCATION = "/notifications/%(service)%.status";
var SERVICE_STATUSLOCATION = SERVICE_BASE + "?service=%(service)%&type=status";
/* Location to the service status history file */
var SERVICE_HISTORYLOCATION = SERVICE_BASE + "?service=%(service)%&type=history";
/* Location to the service error file */
var SERVICE_ERRORLOCATION = SERVICE_BASE + "?service=%(service)%&type=error";
...
ServiceSubscriber.prototype = {
    ...
    subscribe: function(name, isMaster, type) {
        ...
        subscription.registerCallback(lazy_apply(
            function(evt, data) {
                ...
                if(latest != null && latest.data.time == data.data.time) return;
	...
	enconsole.log('Poll with interval ' + interval + ' and a delay of: ' + (delay * i));
...
ServiceSubscription.prototype = {
	...
	enconsole.log("Location has not been found. status: " + this.state + " service: " + this.service)
        if(this.state == 'INITIATING') {
            if(this.stateRequestCount[this.state] >= this.maxNotFound) {
		enconsole.log("Retried already " + this.maxNotFound + " times. Giving up -> Switching to STOPPED")
		...
		enconsole.log("type is observe. service is Therefore we stop now since there's no valid status returned")
    ...
    checkForErrors: function() {
        ...
        error_location_found = function(notification) {
             ...
             */
	    enconsole.log("ERRORLOCATION: CHECKING");
	    if (notification.error) {
		if((this.errorOccured && this.error) || this.state == 'STOPPED') {
		    enconsole.log("NO ERROR FOUND, switching to success");
		    this.switchState("SUCCESS");
		}
		return;
	    }
            if(this._locationAge(notification) >= this.maxErrorAge &&
              !this.error) {
        ...
        errorXHR = $.getJSON(this.error_location + '&rand=' + rand(),
    ...
    poll: function(interval, delay) {
        ...
        if(delay > 0) {
	    enconsole.log("Start with a delay of " + delay)
    ...
    serviceEndedAlready: function() {
        ...
        history_location_found = function(notification) {
             ...
             */
	    if (notification.error) {
                enconsole.log("Request returned an error");
		return;
	    }
            if(this._locationAge(notification) >= this.maxHistoryAge || 
        ...
        historyXHR = $.getJSON(this.history_location + '&rand=' + rand(),
	...
	if (notification.error) {
	    this.requestInProgress = false;
            ...
            $(document).trigger('subscription.locationNotFound', {subscription: this});
	    return;
	}
    ...
    notificationReceived: function(evt, data) {
        ...
        if(this.state == 'INITIATING') {
             ...
             * else, stop polling.
             */
            if(this._locationAge(notification) <= this.maxStatusAge)
                this.switchState('RECEIVING');
            else {
                this.stop();
                return;
        ...
        if(this.previousNotification == null)
            dispatch = true;
        else if(notification.data.time != this.previousNotification.data.time)
            dispatch = true;
     ...
     *
     * Check the date field of the json structure to determine, when
     * the location was last changed and compare it with the current time.
     */
    _locationAge: function(notification) {
        date = notification.modificationtime
        enconsole.log("date: " + date);
        try {
            time1 = date;
            time2 = (new Date()).getTime() / 1000;
	    enconsole.log("now: " + time2);
	    diff = (time2 - time1)
	    enconsole.log("difference: " + diff);
...
jQuery.fn.extend({
    notification: function(services, custom_options) {
        ...
        this.options = {
            ...
            onFailure: lazy_apply(function(evt, notification, subscription) {
                if ($.isArray(notification.data)) {
                    for (var msgIdx = 0; msgIdx < notification.data.length; msgIdx++) {
                        if (notification.data[msgIdx].type == 'error') {
			    var msg = notification.data[msgIdx].msg;
			    break;
			}
		    }
		} else {
                    var msg = notification.data.msg;
		}
            ...
            onNotification: lazy_apply(function(notification, subscription) {
                __element.find('.content').html(notification.data.msg)
            }, this),
            interval: 1500,
            type: 'observer'
        }
        ...
        var _beforeStop = lazy_apply(function() {
        ...
        /* DA: introduced the custom option "delay" to delay of the given
	 *     amount of seconds the reload of the page. */
        if (this.options.delay) {
            var beforeStop = (function(scope) { return function() {
                setTimeout(_beforeStop, scope.options.delay * 1000);
            }})(this);
        } else {
            var beforeStop = _beforeStop;
        }
        ...
        var reload_content = lazy_apply(function(callback) {
            ...
            enconsole.log('Reloading content from ' + document.location.href 
                          + ' ' + klass);
	    // DA: rewritten using $.ajax instead of $.get, as suggested
	    //     by Simon, so that we can disable caching.
            $.ajax({
	...
	enconsole.log('Start polling with interval ' + this.options.interval);

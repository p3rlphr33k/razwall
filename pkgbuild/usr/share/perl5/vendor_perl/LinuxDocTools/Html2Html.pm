...
#  Copyright (C) 2001, Taketoshi Sano
...
#                 tmpbase, debug and charset.
# Usage:
#
#   &{$html2html->{init}}(ext,img,filename,$htmlopts,$fixrefopts,$globalopts);
#
#   ext                      extension
#   img                      use_images
#   filename                 basename
#   $htmlopts->{'split'}     splitlevel
#                              0 - super page mode
#        		 1 - big page mode
#        		 2 - small page mode
#   $htmlopts->{'header'}    header
#   $htmlopts->{'footer'}    footer
#   $htmlopts->{'toc'}       toclevel
#   $fixrefopts->{filenum}   filenumber
#   $fixrefopts->{lrec}      label
#   $globalopts->{tmpbase}
#   $globalopts->{debug}
#   $globalopts->{charset}
# ---------------------------------------------------------------------------
...
$html2html->{init} = sub {
    ...
    $filecount = 1 + $fixrefsopts->{filenum};
    $lprec     = $fixrefsopts->{lrec};

    $header = $htmlopts->{'header'};
    $footer = $htmlopts->{'footer'};

    $toclevel = $htmlopts->{'toc'};
    if ($toclevel == -1) {
    	if ($splitlevel == 0) {
		$toclevel = 0;
	} else {
    ...
    $tmpbase = $globalopts->{tmpbase};
    $content_file = $tmpbase . ".content";

    $debug   = $globalopts->{debug};
    $charset = $globalopts->{charset};
...
sub heading {
    ...
    if ( defined $html_charset_mapping->{$charset} ){
	$outfh->print(" <meta http-equiv=\"Content-Type\" content=\"text/html; charset=$html_charset_mapping->{$charset}\">\n");
    }
    $outfh->print(" <TITLE>");
    $match = $titlename;
    $match =~ s/<[^>]*>//g;

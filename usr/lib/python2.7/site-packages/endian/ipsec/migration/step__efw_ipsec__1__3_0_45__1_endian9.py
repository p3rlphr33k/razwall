...
def main():
    ...
    try:
        ...
        for row in reader:
            ...
            row = row + [""] * (32-len(row))
            if old_length < 32:
                row[31] = 1 # ike version
            if not row[24] or row[24] == 'off':
                row[24] = "" # ESP group type 
            writer.writerow(row)
    finally:

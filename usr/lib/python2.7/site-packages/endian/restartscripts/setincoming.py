...
SETTINGS='/var/efw/incoming/settings'
CONFIG='/var/efw/incoming/config'
ETHERNET_SETTINGS='/var/efw/ethernet/settings'
STATE='/etc/firewall/incoming/iptablesincoming'
TMPFILE="/var/tmp/iptablesincoming"
REGISTRY='/var/lib/ticketregistry/physdevout'
NEEDRELOAD='/var/efw/incoming/needreload'
LOG_SETTINGS='/var/efw/logging/settings'
HOTSPOT_SETTINGS='/var/efw/hotspot/settings'

TMPL='/etc/firewall/incoming/rules.tmpl'

DEFINITIONS = {
    'PHYSDEV_BITS' : 19,
    'PHYSDEV_ENABLE_BIT' : 0x40000,
    'PHYSDEV_MARK' : 0xfff80000
    }

class SetIncoming(Job):

    _uplink_pool = None    
    _red_interfaces_cache = None
    _bridge_config_cache = None
    _ticket_registry = None
    settings = None
    script = None
    tmpl = None
    ...
    def get_bridge_by_interface(self, iface):
        if self._bridge_config_cache == None:
            self._bridge_config_cache = self.get_bridge_config()
        return self._bridge_config_cache[iface]
    
    def get_next_mark(self, hash):
        from endian.firewall.ticket import TicketRegistry
        obj = self._ticket_registry
        if self._ticket_registry == None:
    ...
    def insert_rule(self, obj, log=False):
        methods = {}
        methods['get_bridge_by_interface'] = self.get_bridge_by_interface
        methods['get_next_mark'] = self.get_next_mark
    
        obj.log = log
        self.satanize_rule(obj)
        cmd = str(self.tmpl(namespaces = [obj, methods, DEFINITIONS]))
    ...
    def get_rules(self):
        ...
        try:
            rules=CsvConfigFile(
                '/usr/lib/efw/incoming/config.config',
                CONFIG,
                tolerant=True,
                )
            ret.extend(rules)
    ...
    def start(self, options):
        ...
        self.info("Restart INCOMING firewall, force: %s", options.get('force'))
    ...
    def firewall_restart(self, options):
        self.info("Firewall restart setincoming")
        options['force'] = True
        self.start(options)

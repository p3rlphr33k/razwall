...
CONFIG='/var/efw/queuefw/config'
STATE='/etc/firewall/queuefw/iptablesqueuefw'
TMPFILE="/var/tmp/iptablesqueuefw"
ETHERNET_SETTINGS='/var/efw/ethernet/settings'
NEEDRELOAD='/var/efw/queuefw/needreload'
LOG_SETTINGS='/var/efw/logging/settings'
PROCESS_LISTENING_STATE='/var/run/efw/queuefw/'

TMPL='/etc/firewall/queuefw/rules.tmpl'


class SetQueuefw(Job):
    ...
    def get_rules(self):
        ...
        try:
            rules=CsvConfigFile(
                '/usr/lib/efw/queuefw/config.config',
                CONFIG,
                tolerant=True,
                )
            ret.extend(rules)
    ...
    def check_pid(self, pid):
        ...
        if pid not in self._pid_cache:
            self._pid_cache[pid] = os.path.exists(pid)
    ...
    def generate_script(self):
        ...
        for rule in self.get_rules():
            ...
            if rule.enabled != 'on':
                continue
            if not self.check_pid(rule.pidfile):
                continue
            log = False
            # ICMP hack
            if rule.proto == 'icmp':
    ...
    def start(self, options):
        ...
        self.info("Restart QUEUE firewall, force: %s", options.get('force'))
    ...
    def firewall_restart(self, options):
        self.info("Firewall restart setqueuefw")
        options['force'] = True
        self.start(options)

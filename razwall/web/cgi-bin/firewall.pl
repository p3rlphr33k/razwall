...
my %conf_defaults;
$conf_defaults{conf_dir}    = '/razwall/firewall';
...
# read config files
$conf_file = coalesce($conf_file, '/razwall/firewall/firewall.conf');
...
sub read_rules_file {
  ...
  foreach my $line (@lines) {
    ...
    # the rest of the rule is ready to pass to ipt4() or ipt6()
      ...
      $raw_rule = sprintf('%s -m comment --comment "razwall line %s"', $raw_rule, $line_cnt);

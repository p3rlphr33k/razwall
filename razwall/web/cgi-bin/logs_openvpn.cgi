...
my $logfile="/var/log/openvpn/openvpn.log";
my @files = sort(glob("/var/log/archives/openvpn/openvpn.log-*.gz"));
my $name = "openvpn";
...
my $filestr = $logfile;
if ($date ne $today) {
    $filestr="/var/log/archives/openvpn/openvpn.log-".dateToFilestring($date).".gz";
}
...
;
    ...
    if ($filter eq '') {
	print _("Endian Firewall log of day %s.", $date);
    } else {
	print _("Endian Firewall log of day %s with filter '%s'.", $date, $filter);
    }
    print "\n";
    print "\n";
...
$action2 = _('Sign log');
$ts_file_gz = "/var/log/archives/openvpn/openvpn.log-".dateToFilestring($date).".gz";
$ts_file_tsr = "/var/log/archives/openvpn/openvpn.log-".dateToFilestring($date).".tsr";
$ts_file_zd = "/var/log/archives/openvpn/openvpn.log-".dateToFilestring($date).".zd";
...
&openpage(_('OpenVPN log'), 1, $extraheaders);

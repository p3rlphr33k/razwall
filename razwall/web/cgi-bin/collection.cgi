...
for (qw(action host plugin plugin_instance type type_instance timespan width height title))
{
	$Args->{$_} = param ($_);
}
...
sub action_show_graph
{
  ...
  #my $squid_types = {if_octets => 'if_octets_squid', http_requests => 'apache_requests',
  #	    cache_result => 'connections', cache_ratio => 'gauge'};

  #print STDERR Data::Dumper->Dump ([$Args], ['Args']);

  if ($plugin eq "snmp" && $type_instance eq "squid") {
    #$type = $squid_types->{$type} || $type;
    $type = "${type}_squid";
  }

  # FIXME
  elsif (exists ($MetaGraphDefs->{$type}))
  {
    my %types = _find_types ($host, $plugin, $plugin_instance);
    return $MetaGraphDefs->{$type}->($host, $plugin, $plugin_instance, $type, $types{$type},
				      $width, $height, $img_title);
  }
  ...
  . "/$type_for_file_name" . (defined ($type_instance) ? "-$type_instance" : '');
  ...
  for (my $i = 0; $i < @DataDirs; $i++)
  {
    ...
    RRDs::graph ('-', '-a', 'PNG', '-s', $start_time, '-t', $img_title || $title, '-w', $width, '-h', $height, @RRDDefaultArgs, @rrd_args);
...
sub load_graph_definitions
{
  ...
  $GraphDefs =
  {
   ...
   apache_connections => ['DEF:min={file}:count:MIN',
    ...
    "AREA:max#$HalfBlue",
    "AREA:min#$Canvas",
    "LINE1:avg#$FullBlue:" . _("Percentage"),
    'GPRINT:min:MIN:%6.2lf%% Min,',
    'GPRINT:avg:AVERAGE:%6.2lf%% Avg,',
    'GPRINT:max:MAX:%6.2lf%% Max,',
    ...
    "LINE1:free_acc#$FullGreen:" .sprintf("%-15s", _('Free')),
    'GPRINT:free_min:MIN:%5.1lf%sB ' . _('Minimum'),
    'GPRINT:free_avg:AVERAGE:%5.1lf%sB ' . _('Average'),
    'GPRINT:free_max:MAX:%5.1lf%sB ' . _('Maximum'),
    'GPRINT:free_avg:LAST:%5.1lf%sB ' . _('Current') . '\l',
    "LINE1:used_acc#$FullRed:" .sprintf("%-15s", _('Used')),
    'GPRINT:used_min:MIN:%5.1lf%sB ' . _('Minimum'),
    'GPRINT:used_avg:AVERAGE:%5.1lf%sB ' . _('Average'),
    'GPRINT:used_max:MAX:%5.1lf%sB ' . _('Maximum'),
    'GPRINT:used_avg:LAST:%5.1lf%sB ' . _('Current') . '\l',
    ],
    disk => [
    ...
    percent => ['-v', _('Percentage'),
    'DEF:avg={file}:percent:AVERAGE',
    'DEF:min={file}:percent:MIN',
    'DEF:max={file}:percent:MAX',
    "AREA:max#$HalfBlue",
    "AREA:min#$Canvas",
    "LINE1:avg#$FullBlue:Percentage",
    ...
    "AREA:inc_avg#$HalfBlue",
    "AREA:overlap#$HalfBlueGreen",
    "LINE1:out_avg#$FullGreen:" .sprintf("%-25s", _('Outgoing traffic')),
    'GPRINT:out_avg:AVERAGE:%5.1lf%s ' . _('Average'),
    'GPRINT:out_max:MAX:%5.1lf%s ' . _('Maximum'),
    'GPRINT:out_avg:LAST:%5.1lf%s ' . _('Current'),
    'GPRINT:out_avg_sum:LAST:(~ %5.1lf%sB ' . _('Total') . ')\l',
    "LINE1:inc_avg#$FullBlue:" .sprintf("%-25s", _('Incoming traffic')),
    #'GPRINT:inc_min:MIN:%5.1lf %s Min,',
    'GPRINT:inc_avg:AVERAGE:%5.1lf%s ' . _('Average'),
    'GPRINT:inc_max:MAX:%5.1lf%s ' . _('Maximum'),
    'GPRINT:inc_avg:LAST:%5.1lf%s ' . _('Current'),
    'GPRINT:inc_avg_sum:LAST:(~ %5.1lf%sB ' . _('Total') . ')\l'
    ],

    # SQUID GRAPHS.
    http_requests_squid => ['-v', _('HTTP requests/s'),
    'DEF:min={file}:value:MIN',
    'DEF:avg={file}:value:AVERAGE',
    'DEF:max={file}:value:MAX',
    "AREA:max#$HalfBlue",
    "AREA:min#$Canvas",
    "LINE1:avg#$FullBlue:" . _("HTTP requests"),
    #'GPRINT:min:MIN:%6.2lf' 'Min,',
    'GPRINT:avg:AVERAGE:%6.2lf ' . _('Average'),
    'GPRINT:max:MAX:%6.2lf ' . _('Maximum'),
    'GPRINT:avg:LAST:%6.2lf ' . _('Current') . '\l'
    ],
    cache_result_squid => ['-v', _('Cache hits/s'),
    'DEF:avg={file}:value:AVERAGE',
    'DEF:min={file}:value:MIN',
    'DEF:max={file}:value:MAX',
    "AREA:max#$HalfBlue",
    "AREA:min#$Canvas",
    "LINE1:avg#$FullBlue:" . _('Cache hits'),
    #'GPRINT:min:MIN:%4.1lf Min,',
    'GPRINT:avg:AVERAGE:%4.1lf ' . _('Average'),
    'GPRINT:max:MAX:%4.1lf ' . _('Maximum'),
    'GPRINT:avg:LAST:%4.1lf ' . _('Current') . '\l'
    ],
    cache_ratio_squid => ['-v', _('Cache hits ratio'),
    'DEF:temp_avg={file}:value:AVERAGE',
    'DEF:temp_min={file}:value:MIN',
    'DEF:temp_max={file}:value:MAX',
    "AREA:temp_max#$HalfBlue",
    "AREA:temp_min#$Canvas",
    "LINE1:temp_avg#$FullBlue:" .  _('Cache hits ratio'),
    #'GPRINT:temp_min:MIN:%6.2lf Min,',
    'GPRINT:temp_avg:AVERAGE:%6.2lf ' . _('Average'),
    'GPRINT:temp_max:MAX:%6.2lf ' . _('Maximum'),
    'GPRINT:temp_avg:LAST:%6.2lf ' . _('Current') . '\l'
    ],
    if_octets_squid => ['-v', _('KB/s'), '-l', '0',
    'DEF:out_min_raw={file}:tx:MIN',
    'DEF:out_avg_raw={file}:tx:AVERAGE',
    'DEF:out_max_raw={file}:tx:MAX',
    'DEF:inc_min_raw={file}:rx:MIN',
    'DEF:inc_avg_raw={file}:rx:AVERAGE',
    'DEF:inc_max_raw={file}:rx:MAX',
    'CDEF:out_avg=out_avg_raw,1,*',
    'CDEF:out_max=out_max_raw,1,*',
    'CDEF:inc_avg=inc_avg_raw,1,*',
    'CDEF:inc_max=inc_max_raw,1,*',
    'CDEF:overlap=out_avg,inc_avg,GT,inc_avg,out_avg,IF',
    'CDEF:mytime=out_avg_raw,TIME,TIME,IF',
    'CDEF:sample_len_raw=mytime,PREV(mytime),-',
    'CDEF:sample_len=sample_len_raw,UN,0,sample_len_raw,IF',
    ...
    'GPRINT:inc_max:MAX:%5.1lf%s ' . _('Maximum'),
    'GPRINT:inc_avg:LAST:%5.1lf%s ' . _('Current'),
    'GPRINT:inc_avg_sum:LAST:(~%5.1lf%s KB ' . _('Total') . ')\l'
    ],
...
sub meta_graph_generic_stack
{
  ...
  my @cmd = ('-', '-a', 'PNG', '-s', $timespan_int, '-w', $opts->{'width'},
    '-h', $opts->{'height'},
    '-t', $opts->{'img_title'} || $opts->{'title'} || 'Unknown title',
  ...
  for (my $i = 0; $i < @$sources; $i++)
  {
    ...
    my $legend = sprintf ('%-*s', $max_inst_name, _($inst_name));
    ...
    push (@cmd, qq(AREA:${vname}_stk#$area_color),
      ...
      qq(GPRINT:${vname}_min:MIN:$number_format $min_tr),
      qq(GPRINT:${vname}_avg:AVERAGE:$number_format $avg_tr),
      qq(GPRINT:${vname}_max:MAX:$number_format $max_tr),
      qq(GPRINT:${vname}_avg:LAST:$number_format $cur_tr\\l),
    );
  }
...
sub meta_graph_cpu
{
  confess ("Wrong number of arguments") if (@_ != 8);
  ...
  $opts->{'width'} = $width;
  $opts->{'height'} = $height;
  $opts->{'img_title'} = $img_title;
  $opts->{'title'} = "$host/$plugin"
  . (defined ($plugin_instance) ? "-$plugin_instance" : '') . "/$type";

  $opts->{'rrd_opts'} = ['-v', _('Percentage')];

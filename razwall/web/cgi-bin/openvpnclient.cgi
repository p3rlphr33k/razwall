...
my $ENABLED_PNG = '/toscawidgets/resources/endian.emi/static/images/action_enable_on.png';
my $DISABLED_PNG = '/toscawidgets/resources/endian.emi/static/images/action_enable_off.png';
my $EDIT_PNG = '/toscawidgets/resources/endian.emi/static/images/action_edit.png';
my $DELETE_PNG = '/toscawidgets/resources/endian.emi/static/images/action_delete.png';
...
;
	...
	# Workaround for the configuration downloaded from openvpn
	# access server
	    if ($conf->{'DEVICE'} eq 'tun'){
	    	if (system('/sbin/ifconfig tun0 >/dev/null 2>&1') == 0) {
...
sub import_pem($) {
    ...
    my ($fh, $tmpfile) = tempfile("import.XXXXXX", DIR=>'/var/tmp/', SUFFIX=>'.pem');
...
;
  ...
  <tr>
    ...
    <td>
      <input type="file" name="IMPORT_FILE" size='40' %s/>
    </td>
  </tr>
  ...
  <table cols="2">
    ...
    <tr>
      ...
      <td>
        ...
        <select name="DEV_TYPE" id="advancedDeviceType" onchange="showHideRouteType();">
        ...
        <script type="text/javascript">
	        function showHideBridge() {
	        	...
	        	if (value == 'routed') {
					...
					\$('#natOut').css({'display':'table-row'});
	        	} else {
	        		\$('#advancedBridgedTo').css({'display':'table-row'});
				\$('#natOut').css({'display':'none'});
	        	}
	        }
	        \$(document).ready(function () {showHideBridge();});
        </script>
        <select name="ROUTETYPE" id="advancedRouteType" onchange="showHideBridge();">
...
;
    ...
    <tr valign='top'>
      ...
      <td>
        ...
        <select id="advancedProtocolSelection" name="PROTOCOL" onchange="showHideProxy();">
	...
	<tr id="advancedConfigurationProxy">
		<td colspan="2">
			<table class="list" style="padding: 0px; width:100%; border: 0px;">
			    ...
			    <tr valign='top'>
			      <td>%s:</td>
			      <td><input type='checkbox' name='PROXY_NTLM' value='on' $ntlmchecked{$config{'PROXY_NTLM'}}/></td>
			    </tr>
			    <tr valign='top'>
			      <td>%s:&nbsp;<a href="javascript:void(0);" onmouseover="return overlib('%s',STICKY, MOUSEOFF);" onmouseout="return nd();">?</a></td>
...
sub delete_connection($) {
    ...
    system("/sbin/jobcontrol stop openvpnclients --name=\"$name\" &>/dev/null");
...
sub store_config($$$) {
    ...
    if ($fileconf{'AUTH_CERT'} eq 'on') {
		if (! -e $openvpn_dir.$name.'/certs.p12' && ! $demo) {
...
sub add_tlsauth() {
    ...
    if (ref ($par{'IMPORT_FILE'}) eq 'Fh') {
        my ($fh, $tmpfile) = tempfile("import.XXXXXX", DIR=>'/var/tmp/', SUFFIX=>'.key');
        if (!copy ($par{'IMPORT_FILE'}, $fh)) {

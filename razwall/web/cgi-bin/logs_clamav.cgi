...
my @files = sort(glob("/var/log/archives/clamav/clamd.log-*.gz"));
my $name = "clamav";
...
my $filestr = $logfile;
if ($date ne $today) {
    $filestr="/var/log/archives/clamav/clamd.log-".dateToFilestring($date).".gz";
}
...
$action2 = _('Sign log');
$ts_file_gz = "/var/log/archives/clamav/clamd.log-".dateToFilestring($date).".gz";
$ts_file_tsr = "/var/log/archives/clamav/clamd.log-".dateToFilestring($date).".tsr";
$ts_file_zd = "/var/log/archives/clamav/clamd.log-".dateToFilestring($date).".zd";
...
&openpage(_('%s Antivirus log', 'ClamAV'), 1, $extraheaders);

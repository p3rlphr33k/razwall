...
my @files = sort(glob("/var/log/archives/firewall-*.gz"));
my $name = "firewall";
...
my $filestr = $logfile;
if ($date ne $today) {
    $filestr="/var/log/archives/firewall-".dateToFilestring($date).".gz";
}
...
$action2 = _('Sign log');
$ts_file_gz = "/var/log/archives/firewall-".dateToFilestring($date).".gz";
$ts_file_tsr = "/var/log/archives/firewall-".dateToFilestring($date).".tsr";
$ts_file_zd = "/var/log/archives/firewall-".dateToFilestring($date).".zd";
...
&oldernewer();
printf <<END
<table width='100%' class="ruleslist" cellspacing="0" cellpadding="0" style="font-size: 9px">
<tr>
        ...
        <td width='15%' align='center' class='boldbase'><b>%s</b></td>
        <td width='5%' align='center' class='boldbase'><b>%s</b></td>
        <td width='21%' align='center' class='boldbase'><b>%s</b></td>
        <td width='21%' align='center' class='boldbase'><b>%s</b></td>
        <td width='5%' align='center' class='boldbase'><b>%s</b></td>
</tr>
END
...
foreach my $line (@slice) {
	...
	<td align='center'><a href='http://isc.sans.org/port_details.php?port=$srcport' target='top'>$printsrcport</a></td>
	...
	<td align='center'><a href='http://isc.sans.org/port_details.php?port=$dstport' target='top'>$printdstport</a></td>
        </tr>
END
;
    $i++;

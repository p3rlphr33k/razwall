...
<table width="100%" cellpadding="0" cellspacing="0">
  <tr>
    <td>
        <div class="editoradd" style="margin-bottom: 0px;">
            <a href="#" onclick="swapVisibility('defaultedit')">%s</a>
        </div>
    </td>
    <td align="right">
      <form method="post" >
...
;
    ...
    my $remark = value_or_nbsp($splitted{'remark'});
        ...
        if ($splitted{'user'} =~ /^$/) {
            $user = "<i>".value_or_nbsp($config->{'DEFAULT_USER'})."</i>";
        }
        if ($splitted{'spam'} =~ /^$/) {
...
sub display_default() {
    ...
    print "<div id=\"defaultedit\" style=\"display: none\">\n";
...
<form action="$ENV{'SCRIPT_NAME'}" method="post" enctype='multipart/form-data'>
  ...
  <table width="100%" columns="4">
    <tr>
      <td>%s</td>
      <td><input type='text' name='DEFAULT_HOST' size='30' value="$host" /></td>
      <td colspan="2">&nbsp;</td>
    </tr>
    <tr>
      <td>%s</td>
      <td><input type='text' name='DEFAULT_USERNAME' size='20' value="$user" /></td>
      <td>%s</td>
      <td><input type='password' name='DEFAULT_PASSWORD' size='20' value="$password" /></td>
    </tr>
    <tr>
      <td>%s</td>
      <td><input type='text' name='DEFAULT_HAMFOLDER' size='20' value="$ham" /></td>
      <td>%s</td>
      <td><input type='text' name='DEFAULT_SPAMFOLDER' size='20' value="$spam" /></td>
    </tr>
    <tr>
      <td colspan="4">&nbsp;</td>
    </tr>
...
sub save() {
	...
	store_config();
	`$restartspamassassin --reschedule`;
	return;
    }
...
<table columns="4" width="100%">
  ...
  <tr valign='top'>
    <td colspan="2">
      <input class='submitbutton' type='submit' name='save_update' value='%s' />
    </td>
    <td colspan="2">&nbsp;</td>
  </tr>
</table>
...
&openbox('100%', 'left', _('Current spam training sources'));

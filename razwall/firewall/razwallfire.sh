...
done <<< $(sed -e 's|\s||g' -e '/^#/d' /razwall/firewall/firewall.conf | tr '[A-Z]' '[a-z]')
if [[ $ipv4 -eq 0 ]] && [[ $ipv6 -eq 0 ]] ; then
  echo "Need to enable at least 1 of IPv4 or IPv6 in firewall.conf"
  exit 3
fi
...
if [[ $ipv4 -eq 1 ]] ; then
  ...
  compile_rules 'firewall -4' $IPv4_FILE
fi
if [[ $ipv6 -eq 1 ]] ; then
  echo '   => IPv6'
  logger -t fwfire -p user.info -- 'Compiling IPv6 rules'
  compile_rules 'firewall -6' $IPv6_FILE
fi
